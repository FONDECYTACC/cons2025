...
#
# =============================================================================
# _alt_scripts VERSION — audited + competing-risk (Aalen-Johansen) observed risk
# for READMISSION, Kaplan-Meier kept for DEATH. Reviewed 2026-05-30.
#
# WHY THIS VERSION EXISTS
#   The original (cons/_hist_scripts/adca_from_results_boot.R) estimates the
#   OBSERVED event risk in each threshold-positive subgroup with 1 - KM, which
#   treats the competing event (death) as independent censoring. For READMISSION
#   with non-negligible mortality, 1 - KM OVERESTIMATES the cumulative incidence
#   and the overestimation grows with the horizon. The project's calibration
#   already uses Aalen-Johansen (AJ) with death as a competing event
#   (cons/_alt_scripts/calibrate_cscox_aj_grouped_corr.R). This version makes the
#   DCA observed risk for readmission consistent with that calibration.
#
# WHAT CHANGED vs the _hist_scripts version
#   1. READMISSION observed risk = AJ cause-specific CIF (cause = readmission),
#      computed exactly as in calibrate_cscox_aj_grouped_corr()$.aj_cif_readmit:
#         survfit(Surv(.ftime, factor(.fstatus)) ~ 1)  ->  pstate["readmit"].
#      DEATH observed risk = 1 - KM, UNCHANGED (death is the terminal event; no
#      competing event to adjust for, per the user's instruction).
#   2. The competing-risk first-event status (.ftime, .fstatus in {0,1,2}) is
#      reconstructed PER REPLICATE by combining the readmission and death y_val
#      blocks that evaluate_dual_cox_python_style stores for the SAME fold. No
#      refit, no original data needed. Requires the two blocks to be row-aligned
#      (same fold => same strata-based row dropping). Guarded at runtime:
#      if the death block is absent/errored or row counts differ, that replicate
#      transparently FALLS BACK to 1 - KM and the count is reported.
#   3. Efficiency: within each (replicate, horizon) the threshold-positive sets
#      are nested (predicted risk is fixed; thresholds are sorted). The observed
#      estimator (AJ or KM) is recomputed ONLY when the positive set actually
#      changes (dedup by |positive set|). The all-sample ("treat all") risk is
#      computed once per horizon. This cuts survfit calls substantially and keeps
#      AJ (heavier than KM) affordable. KM (death) numbers are bit-identical to
#      the original; dedup only avoids redundant refits.
#   4. do.call(rbind, .) -> do.call(rbind.data.frame, .) everywhere. With `mice`
#      attached, the bare rbind generic can dispatch to rbind.mids and fail on
#      list inputs (documented failure mode in the calibration work).
#
# Net-benefit math is otherwise IDENTICAL to the original (validated):
#   NB = p_pos * (risk_pos - (1 - risk_pos) * pt/(1-pt)), per McLernon 2023.
#
# Drop-in: same function name, same output contract ($summary, $focus_windows,
# $raw, $config). New optional args have safe defaults, so existing callers
# (compute_dca_from_boot / run_dca_full_summary) work unchanged.
# =============================================================================
#
# Usage:
...
# Locate a target state column in a survfit multistate pstate object.
# Ported from calibrate_cscox_aj_grouped_corr() for consistency.
...
# Aalen-Johansen cause-specific cumulative incidence of READMISSION (cause 1)
# from a competing-risk first-event encoding:
#   fstatus: 0 = censored, 1 = readmission first, 2 = death first.
# Mirrors .aj_cif_readmit() in calibrate_cscox_aj_grouped_corr().
.aj_readmit_cif <- function(ftime, fstatus, horizon) {
    if (!length(ftime) || !any(fstatus == 1L, na.rm = TRUE)) {
        return(0)
    }
    sf <- factor(fstatus, levels = c(0L, 1L, 2L), labels = c("censor", "readmit", "death"))
    fit <- tryCatch(
        survival::survfit(survival::Surv(ftime, sf) ~ 1),
        error = function(e) NULL
    )
    if (is.null(fit)) return(NA_real_)
    s <- summary(fit, times = horizon, extend = TRUE)
    ps <- s$pstate
    if (is.null(ps) || !length(ps)) return(0)
    pos <- tryCatch(.state_position(ps, fit$states, "readmit"), error = function(e) NA_integer_)
    if (is.na(pos)) return(NA_real_)
    risk <- if (is.null(dim(ps))) as.numeric(ps[pos]) else as.numeric(ps[1, pos])
...
# Extract per-replicate prediction blocks for one risk. When attach_competing is
# TRUE (readmission + AJ), also reconstruct the competing-risk first-event
# (.ftime/.fstatus) from the SAME item's death block, when row-aligned.
.extract_replicates <- function(results_boot,
                                risk = c("readmission", "death"),
                                attach_competing = FALSE,
                                competing_risk = "death",
    ...
    for (item in raw_predictions) {
        ...
        cr_fstatus <- NULL
        has_competing <- FALSE
        if (isTRUE(attach_competing)) {
            comp <- item[[competing_risk]]
            comp_ok <- is.list(comp) &&
                !("error" %in% names(comp) && nzchar(comp$error %||% "")) &&
                !is.null(comp$y_val) &&
                all(c("time", "event") %in% names(comp$y_val)) &&
                nrow(comp$y_val) == nrow(y_val)
            if (isTRUE(comp_ok)) {
                rt <- as.numeric(y_val$time)
                re <- as.integer(y_val$event)
                dt <- as.numeric(comp$y_val$time)
                de <- as.integer(comp$y_val$event)
                ftime <- pmin(rt, dt)
                ...
                both_at_first <- r_first & d_first
                if (any(both_at_first)) {
                    fstatus[both_at_first] <- if (tie_action == "readmit_first") 1L else 2L
                }
                cr_ftime <- ftime
                cr_fstatus <- fstatus
                has_competing <- TRUE
    ...
    attr(blocks, "n_fallback") <- n_fallback
    blocks
}
...
.compute_replicate_curves <- function(block, thresholds, horizons,
    ...
    use_aj <- identical(block$risk, "readmission") &&
        identical(readmit_method, "aalen-johansen") &&
        isTRUE(block$has_competing)
    ...
    for (horizon in horizons) {
        ...
        if (isTRUE(use_aj)) {
            ft0 <- block$cr_ftime
            fs0 <- block$cr_fstatus
            keep <- is.finite(risk_vec) & is.finite(ft0) & !is.na(fs0)
            if (!any(keep)) {
                next
            }
            ...
            ft <- ft0[keep]
            fs <- fs0[keep]
            observed <- function(sel) .aj_readmit_cif(ft[sel], fs[sel], horizon)
        } else {
            time <- block$time
            event <- block$event
            ...
            tt <- time[keep]
            ev <- event[keep]
            observed <- function(sel) .km_event_risk(tt[sel], ev[sel], horizon)
        }

        n_obs <- length(risk_h)
        ord <- order(risk_h, decreasing = TRUE)
        all_event_risk <- observed(seq_len(n_obs))

        # Nested positive sets: recompute observed risk only when |positive| changes.
        prev_n <- NA_integer_
        cached_risk <- NA_real_
        for (threshold in thresholds) {
            selected_n <- sum(risk_h >= threshold)
            positive_rate <- selected_n / n_obs

            if (!is.na(prev_n) && selected_n == prev_n) {
                model_event_risk <- cached_risk
            } else {
                model_event_risk <- if (selected_n == 0L) 0 else observed(ord[seq_len(selected_n)])
                cached_risk <- model_event_risk
                prev_n <- selected_n
            }
...
.collapse_curves_long <- function(raw_curves) {
    ...
    do.call(rbind.data.frame, pieces)
}
...
.summarize_curves <- function(curves_long) {
    ...
    out <- do.call(rbind.data.frame, summary_list)
...
.plot_adca <- function(summary_curves, focus_windows, output_file) {
    ...
    p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = threshold, y = net_benefit_mean, color = strategy)) +
        ...
        ggplot2::labs(
            ...
            subtitle = "Readmission: Aalen-Johansen observed risk (death as competing event). Death: Kaplan-Meier."
        ) +
...
# results_boot      a results_boot object, or NULL with results_boot_path set
# readmit_method    "aalen-johansen" (default) competing-risk CIF for readmission,
#                   or "km" to force 1 - KM (reproduces the _hist_scripts behavior)
# tie_action        how to resolve simultaneous readmission/death first events
#                   when reconstructing the competing-risk status ("death_first"
#                   default, matching calibrate_cscox_aj_grouped_corr)
run_adca_from_results_boot <- function(
    ...
    save_raw = TRUE,
    create_plot = TRUE,
    readmit_method = c("aalen-johansen", "km"),
    tie_action = c("death_first", "readmit_first"),
    verbose = TRUE
) {
    ...
    use_aj <- identical(readmit_method, "aalen-johansen")
    readmission_blocks <- .extract_replicates(
        results_boot,
        risk = "readmission",
        attach_competing = use_aj,
        competing_risk = "death",
        tie_action = tie_action
    )
    death_blocks <- .extract_replicates(results_boot, risk = "death")

    n_readmit_total <- length(readmission_blocks)
    n_readmit_fallback <- attr(readmission_blocks, "n_fallback") %||% 0L
    n_readmit_aj <- sum(vapply(readmission_blocks, function(b) isTRUE(b$has_competing), logical(1L)))
    if (use_aj && isTRUE(verbose)) {
        message(sprintf(
            "Readmission observed risk: Aalen-Johansen on %d/%d replicate(s)%s. Death: Kaplan-Meier.",
            n_readmit_aj, n_readmit_total,
            if (n_readmit_fallback > 0L) {
                sprintf("; %d replicate(s) fell back to 1-KM (no row-aligned death block)", n_readmit_fallback)
            } else ""
        ))
    }
    ...
    raw_curves_list <- lapply(
        all_blocks, .compute_replicate_curves,
        thresholds = thresholds, horizons = horizons,
        readmit_method = readmit_method
    )
    raw_curves <- do.call(rbind.data.frame, Filter(nrow, raw_curves_list))
    if (!nrow(raw_curves)) {
    ...
    result <- list(
        ...
        config = list(
            ...
            horizons = horizons,
            readmit_observed_method = readmit_method,
            death_observed_method = "km",
            tie_action = tie_action,
            n_readmit_replicates = n_readmit_total,
            n_readmit_aalen_johansen = n_readmit_aj,
            n_readmit_km_fallback = n_readmit_fallback,
            reference = "McLernon et al. 2023; doi:10.7326/M22-0844",
            note = paste(
                "DCA at fixed horizons over a broad threshold grid.",
                "Readmission observed risk = Aalen-Johansen cause-specific CIF (death competing);",
                "death observed risk = 1 - Kaplan-Meier.",
                "Focus window is a project-specific heuristic fallback (no external clinical threshold supplied)."
            ),
