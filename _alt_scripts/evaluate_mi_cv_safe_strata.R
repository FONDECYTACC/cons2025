...
# PATCHED VERSION (2026-05-26):
#   1. df_list trimmed to required_cols before future_lapply
#   2. df_list_raw + imputed_list rm()'d from closure env before lambda definition
#   3. future.globals = list(helpers only) — disables auto-scan, prevents df_list double-export
#   4. caret::createFolds replaced with base-R stratified fold generation
#
# Usage:
...
evaluate_mi_cv_safe_strata <- function(
    ...
    # -- PATCH 1: trim df_list to required_cols only; free large objects from closure --
    df_list <- lapply(df_list, function(d) d[, intersect(required_cols, names(d)), drop = FALSE])
    rm(df_list_raw)
    rm(imputed_list)
    # ---------------------------------------------------------------------------------
    ...
    # PATCH 2: replace caret::createFolds with base-R stratified fold creation
    folds_by_repeat <- lapply(seq_len(n_repeats), function(rep_id) {
        set.seed(seeds[rep_id])
        sv <- as.integer(cv_strata)
        fold_assign <- integer(length(sv))
        for (s in unique(sv)) {
            idx <- which(sv == s)
            fold_assign[idx] <- sample(rep(seq_len(k), length.out = length(idx)))
        }
        lapply(seq_len(k), function(f) sort(which(fold_assign == f)))
    })
    ...
    if (n_cores > 1L) {
        # Set via options (not plan() args) to avoid "unknown future arguments" warning.
        # parallelly.makeNodePSOCK.rscript_args = "--no-init-file" skips .Rprofile in workers
        # (renv autoload fails in subprocess context on parallelly >= 1.38 ping test).
        options(
    ...
    all_results <- future.apply::future_lapply(
        ...
        future.seed = TRUE,
        # PATCH 3: explicit globals list — disables auto-scan so df_list is not double-exported
        # (df_list is already in the closure, trimmed; only helper functions need explicit export)
        future.globals = list(
            to01                              = to01,
            ibs_ipcw_train                   = ibs_ipcw_train,
            .mi_cv_mean_or_na                = .mi_cv_mean_or_na,
            .mi_cv_prepare_model_split       = .mi_cv_prepare_model_split,
            .mi_cv_collect_observed_levels   = .mi_cv_collect_observed_levels,
            .mi_cv_build_level_mapping       = .mi_cv_build_level_mapping,
            .mi_cv_sanitize_level_name       = .mi_cv_sanitize_level_name,
            .mi_cv_make_strata_signature_safe = .mi_cv_make_strata_signature_safe,
            .mi_cv_extract_strata_args       = .mi_cv_extract_strata_args,
            .mi_cv_rewrite_strata_calls      = .mi_cv_rewrite_strata_calls,
            .mi_cv_normalize_call_name       = .mi_cv_normalize_call_name,
            .mi_cv_coerce_rowwise_value      = .mi_cv_coerce_rowwise_value
        ),
