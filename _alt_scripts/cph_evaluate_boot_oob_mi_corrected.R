# PATCHED VERSION (2026-05-26) — moved from _hist_scripts to _alt_scripts
# Changes vs original:
#   1. options(parallelly.makeNodePSOCK.rscript_args="--no-init-file", ...) before plan()
#      — fixes worker startup crash when .Rprofile activates renv (parallelly >= 1.38)
#   2. df_list trimmed to needed columns only before worker_state is built
#   3. rm(df_list_raw, imputed_list, df_list) before future_lapply
#      — removes large objects from cph_boot_worker's closure frame
#   4. future.chunk.size = ceiling(B / n_cores) — batches tasks, reduces serializations
#   5. min_strata_events added to signature (accepted for API compat, not enforced)
...
cph_evaluate_boot_oob_mi_corrected <- function(
    ...
    n_cores <- as.integer(max(1L, min(cpus, B, detected - 1L)))

    old_plan <- future::plan()
    on.exit(future::plan(old_plan), add = TRUE)
    if (n_cores > 1L) {
        # Set via options (not plan() args) to avoid "unknown future arguments" warning from
        # future package. These global options are honoured by parallelly::makeClusterPSOCK
        # and skip .Rprofile (renv autoload) in worker subprocesses — which causes workers to
        # exit with status 1 on parallelly >= 1.38 (new ping test).
        options(
    ...
    # PATCH: trim df_list to needed columns; free large objects from cph_boot_worker's closure
    needed_cols <- unique(c(all.vars(formula), time_col, event_col))
    df_list <- lapply(df_list, function(d) d[, intersect(needed_cols, names(d)), drop = FALSE])
    rm(df_list_raw)
    rm(imputed_list)
    ...
    # PATCH: rm df_list from outer frame so cph_boot_worker's closure doesn't carry it
    rm(df_list)
    ...
    boot_results <- future.apply::future_lapply(
        ...
        # PATCH: chunk tasks to reduce serializations from B to n_cores
        future.chunk.size = ceiling(B / n_cores),
