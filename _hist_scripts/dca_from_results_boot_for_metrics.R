...
#
#   source("cons/_hist_scripts/dca_from_results_boot_for_metrics.R")
#
...
# horizons           numeric vector; each value must be in
#                    results_boot$config$eval_times (typically c(6,12,36,60))
...
compute_dca_from_boot <- function(
  ...
  for (h in horizons) {
    ...
    for (i in seq_along(results_boot_list)) {
      per_model[[i]] <- .dca_extract_horizon(adca_list[[i]], risk, h, thresholds)
    }
...
plot_dca_multi_horizon <- function(
  ...
  for (h_label in names(dca_multi)) {
    ...
    # Treat all and Treat none come from the first model (same for all)
    first <- h_data[[which(!vapply(h_data, is.null, logical(1L)))[1L]]]
    if (!is.null(first)) {
      base <- data.frame(
        horizon   = paste0(h_label, " months"),
...
# horizons           must be in results_boot$config$eval_times
